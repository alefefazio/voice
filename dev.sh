...
workon voice  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development voice npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop voice
    docker rm voice
    docker run --name voice -d --env-file /home/ubuntu/voice.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/voice:/dkdata \
        voice start_web.sh
}
...
echo_green "Welcome to voice's dev env"
