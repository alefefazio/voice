...
```bash
dk bash  # starts bash inside "voice" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and voice.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `voice` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The voice container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv voice -p python3  # creates a python3 virtualenv
